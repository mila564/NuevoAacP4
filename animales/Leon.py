...
class Leon(Carnivoro):
    def run(self):
        descansar = 5
        while not self.simulacion.ganador.booleano:
            caze = self.cazar()
            ...
            if descansar == 0:
                ...
                sleep(abs(85 - self.velocidad_animal))
                descansar -= 1
        ...
        if self.mi_casilla is not None:
                ...
                for j in range(3):
                    ...
                    if 0 <= x_casilla_candidata < self.simulacion.entorno.dimensiones and 0 <= y_casilla_candidata < self.simulacion.entorno.dimensiones:
                        ...
                        if casilla_candidata is not None and not casilla_candidata.mutex.locked() and (
                                isinstance(casilla_candidata.animal, Cebra) or isinstance(casilla_candidata.animal,
                                                                                          Hiena) and self.superioridad_numerica()):
            ...
            if len(casillas_candidatas) == 0 or self.simulacion.ganador.booleano:
                ...
                if isinstance(animal_cazado, Cebra):
                    ...
                    manada_animal.puntuacion.puntos += 2
                ...
                for j in range(3):
                    ...
                    if 0 <= x_casilla_candidata < self.simulacion.entorno.dimensiones and 0 <= y_casilla_candidata < self.simulacion.entorno.dimensiones:
                        ...
                        if casilla_candidata.animal is not None:
                                if isinstance(casilla_candidata.animal, Leon):
                                    num_leones += 1
                                elif isinstance(casilla_candidata.animal, Hiena):
                                    num_hienas += 1
            return num_leones >= num_hienas
